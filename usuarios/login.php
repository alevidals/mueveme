...
<body>
    ...
    <?php
        ...
        if (es_POST() && empty($errores)) {
            ...
            if ($remember) {
                do {
                    $tokenCookie = md5(uniqid(mt_rand(), true));
                    $sent = $pdo->prepare('SELECT token
                                             FROM usuarios
                                            WHERE token = :token');
                    $sent->execute(['token' => $tokenCookie]);
                } while ($sent->fetch() !== false);

                $sent = $pdo->prepare("UPDATE usuarios
                                          SET token = '$tokenCookie'
                                        WHERE login = :login");
                $sent->execute(['login' => $_SESSION['login']]);

                setcookie("username", $_SESSION['login'], time()+2*24*60*60, '/');
                setcookie("tokenCookie", $tokenCookie, time()+2*24*60*60, '/');
            }
    ...
    <!-- todo entre esto -->
        ...
        <div class="row">
            <div class="col offset-5">
                <img src="https://cdn.pixabay.com/photo/2016/11/18/23/38/child-1837375_960_720.png" alt="avatar" class="avatar">
            </div>
        </div>
        <div class="row mt-2">
